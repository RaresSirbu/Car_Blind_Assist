...
void loop() {
  ...
  if (acum - ultimaCitireStang >= INTERVAL_SENZOR) {
    ...
    distantaStangFiltrata = calculeazaMedie(distanteStang); // ✅ imediat după citire
  ...
  static unsigned long ultimaImprimare = 0;
  if (acum - ultimaImprimare >= 500) {
    ultimaImprimare = acum;
    Serial.println("----- Stare BSM -----");
    Serial.print("Stanga: "); Serial.print(distantaStangFiltrata);
    Serial.print(" cm | Dreapta: "); Serial.print(distantaDreptFiltrata);
    Serial.println(" cm");
  }
}
